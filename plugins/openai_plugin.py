...
class OpenAIPlugin(plugin.Plugin):
    ...
    def _parse_args(self, args):
        ...
        if self.load_balance:       
            self.user_id = 0
            self.host = ""  
            self.endpoint = args.get("endpoint") # will get overwritten
        else:
    ...
    def request_http(self, query: dict, user_id: int, test_end_time: float = 0, host_override: str = ""):
    ...
    def streaming_request_http(self, query: dict, user_id: int, test_end_time: float, host_override: str):
        headers = {"Content-Type": "application/json"}
        ...
        if self.load_balance:
            ...
            if host_override:
                host = "http://" + host_override + self.endpoint
        else:
            host = self.host
